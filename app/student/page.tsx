...
export default function StudentPage() {
    ...
    useEffect(() => {
        ...
        const savedPhoto = localStorage.getItem("gatepass_photo");
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        if (res.error) {
            ...
            if (res.photoUrl) localStorage.setItem("gatepass_photo", res.photoUrl);
        }
        setIsLoading(false);
    };
    ...
    const handleLogout = () => {
        ...
        localStorage.removeItem("gatepass_photo");
        setSecret(null);
        setRollNumber("");
        setStudentName("");
        setPhotoUrl(null);
    ...
    return (
        <main className="min-h-screen flex flex-col items-center justify-center p-4 bg-black relative overflow-hidden">
            <div className="w-full max-w-md bg-gray-950/80 border border-neon-blue/30 backdrop-blur-md rounded-2xl p-8 flex flex-col items-center gap-8 shadow-[0_0_50px_-10px_rgba(15,240,252,0.2)]">
                ...
                {/* Header with Photo */}
                <div className="text-center space-y-4 w-full border-b border-gray-800 pb-6">
                    {/* Student Photo */}
                    <div className="flex justify-center mb-4">
                        <div className="relative w-24 h-24 rounded-full overflow-hidden border-4 border-neon-blue shadow-[0_0_20px_rgba(15,240,252,0.5)]">
                            {photoUrl ? (
                                <img
                                    src={photoUrl}
                                ...
                                <div className="w-full h-full bg-gray-800 flex items-center justify-center text-neon-blue text-3xl font-bold">
                                    {studentName?.charAt(0) || rollNumber?.charAt(0) || "?"}
                                </div>
                            )}
                        </div>
                    </div>
